...
import {
  ...
  Loader2, Upload, Music2, X, Eye, PenLine,
} from "lucide-react";
...
const STEPS = ["Names", "Date & Venue", "Photos", "Features", "Publish", "Preview"];
...
function MusicUpload({
  currentUrl, onUpload, onRemove, accent,
}: {
  currentUrl: string; onUpload: (url: string) => void;
  onRemove: () => void; accent: string;
}) {
  ...
  async function handleFile(file: File) {
    setUploadError("");
    if (file.size > MAX_BYTES) { setUploadError("File must be under 10MB"); return; }
    setFileName(file.name); setUploading(true); setProgress(0);
    const ticker = setInterval(() => setProgress(p => p < 88 ? p + 4 : p), 200);
    try {
      const supabase = createClient();
      const ext  = file.name.split(".").pop() ?? "mp3";
      const path = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
      const { data, error: uploadErr } = await supabase.storage
        .from("invitation-music").upload(path, file, { contentType: file.type, upsert: false });
      clearInterval(ticker);
      if (uploadErr) {
        setUploadError(uploadErr.message); setUploading(false); setProgress(0); return;
      }
      setProgress(100);
      const { data: { publicUrl } } = supabase.storage.from("invitation-music").getPublicUrl(data.path);
      onUpload(publicUrl);
  ...
  if (currentUrl && !uploading) {
    return (
      <div className="flex items-center gap-3 rounded-xl border px-3 py-3"
        ...
        <button type="button" onClick={() => { setFileName(""); onRemove(); }}
                className="shrink-0 p-1 rounded-lg hover:opacity-70" style={{ color: "#b8b4aa" }}>
          <X size={15} />
        </button>
      </div>
    );
    ...
    return (
      <div className="rounded-xl border p-4 space-y-2"
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm truncate" style={{ color: "#FAF6EF" }}>Uploading {fileName}…</p>
        </div>
  ...
  return (
    <div>
      <button type="button" onClick={() => inputRef.current?.click()}
              className="w-full flex items-center gap-3 rounded-xl border px-4 py-3 text-left hover:opacity-80"
        ...
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: "#b8b4aa" }}>MP3, WAV, OGG, M4A · Max 10MB</p>
        </div>
      </button>
...
function PreviewRenderer({ templateId, form }: { templateId: string; form: FormData }) {
  const weddingDate = form.weddingDate
    ? new Date(`${form.weddingDate}T${form.weddingTime || "19:00"}`)
  ...
  const events = form.events
    ...
    .map(e => ({ name: e.name, venue: e.venue, date: new Date(e.date) }));

  const baseProps = {
    groomName:    form.groomName    || "Groom Name",
    brideName:    form.brideName    || "Bride Name",
    weddingDate,
    ...
    shareUrl:  "https://cards.noorapp.app/i/preview",
  };

  if (templateId === "islamic-premium") {
    return <IslamicPremiumTemplate {...baseProps} endTime={endTime} />;
  }
  if (templateId.startsWith("islamic")) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return <IslamicTemplate {...(baseProps as any)} />;
  }
  if (templateId.startsWith("hindu")) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return <HinduTemplate {...(baseProps as any)} />;
  }
  if (templateId.startsWith("christian")) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return <ChristianTemplate {...(baseProps as any)} />;
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return <ModernTemplate {...(baseProps as any)} musicUrl={form.musicUrl || undefined} />;
}
...
export default function CreatePage({ params }: { params: Promise<{ templateId: string }> }) {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    weddingDate: "", weddingTime: "19:00", endTime: "",
  ...
  const formContent = (
    <div className="py-12 lg:py-8 px-4">
      <div className="max-w-lg mx-auto">

        {/* Header */}
        <div className="text-center mb-8">
          <p className="text-2xl mb-1">{template.emoji}</p>
          <h1 className="font-amiri text-3xl mb-1" style={{ color: "#FAF6EF" }}>{template.name}</h1>
          ...
          {STEPS.map((label, i) => (
            <div key={label} className="flex-1 flex flex-col items-center gap-1">
              ...
              <span className="text-xs hidden sm:block"
                    style={{ color: i === step ? accent : "#b8b4aa60" }}>{label}</span>
            </div>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            ...
            {step === 0 && (
              <>
                <Field label="Groom's Name" value={form.groomName} onChange={v => set("groomName", v)}
                       placeholder="Ahmad" required />
                <Field label="Bride's Name" value={form.brideName} onChange={v => set("brideName", v)}
                       placeholder="Mariam" required />
                <Field label="Groom's Family (optional)" value={form.groomFamily}
                       onChange={v => set("groomFamily", v)} placeholder="Son of Ibrahim Al-Rashid" />
                <Field label="Bride's Family (optional)" value={form.brideFamily}
                       onChange={v => set("brideFamily", v)} placeholder="Daughter of Yusuf Al-Amin" />
              </>
            )}
            ...
            {step === 1 && (
              <>
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Wedding Date" value={form.weddingDate}
                         onChange={v => set("weddingDate", v)} type="date" required />
                  <Field label="Start Time" value={form.weddingTime}
                         onChange={v => set("weddingTime", v)} type="time" />
                </div>
                <Field label="End Time (optional)" value={form.endTime}
                       onChange={v => set("endTime", v)} type="time" />
                <Field label="Venue Name" value={form.venue} onChange={v => set("venue", v)}
                       placeholder="Al-Baraka Banquet Hall" required />
                <Field label="Venue Address" value={form.venueAddress}
                       onChange={v => set("venueAddress", v)} placeholder="123 Main St, Dubai, UAE" />
                <Field label="Google Maps URL (optional)" value={form.venueMapUrl}
                       onChange={v => set("venueMapUrl", v)} placeholder="https://maps.google.com/..." />
              </>
            )}
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                {form.photos.map((url, i) => (
                  <div key={i} className="flex gap-2">
                    <input
                      value={url}
                      onChange={e => {
                        const u = [...form.photos]; u[i] = e.target.value; set("photos", u);
                      }}
                ...
                {form.photos.length < 10 && (
                  <button onClick={() => set("photos", [...form.photos, ""])}
                          className="flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium border w-full justify-center"
                          style={{ borderColor: `${accent}50`, color: accent }}>
                    <Plus size={15} /> Add Photo URL
                  </button>
                )}
                <div className="rounded-xl p-4 flex items-start gap-3 text-xs"
                  ...
                  <span>Upload photos to Imgur or Cloudinary, then paste the direct image URLs here. Supports JPG, PNG, WebP.</span>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-5">
                ...
                <div className="flex items-center justify-between rounded-xl p-3"
                  ...
                  <button onClick={() => set("showHijriDate", !form.showHijriDate)}
                ...
                <Field label="Dress Code (optional)" value={form.dressCode}
                       onChange={v => set("dressCode", v)} placeholder="Formal / Traditional" />
                <Field label="Transport Info (optional)" value={form.transport}
                       onChange={v => set("transport", v)} placeholder="Free bus from Grand Mosque" />

                <div>
                  <p className="text-xs font-medium mb-1.5" style={{ color: "#b8b4aa" }}>
                    Background Music (optional)
                  </p>
                  <MusicUpload currentUrl={form.musicUrl}
                ...
                {/* Events */}
                <div>
                  <p className="text-xs font-medium mb-2" style={{ color: "#b8b4aa" }}>
                    Pre-wedding Events (optional)
                  </p>
                  {form.events.map((ev, i) => (
                    <div key={i} className="rounded-xl p-3 mb-3 space-y-2"
                         style={{ background: "#0f1f17", border: "1px solid #234d38" }}>
                      <input value={ev.name}
                             onChange={e => {
                               const u = [...form.events]; u[i] = { ...u[i], name: e.target.value }; set("events", u);
                             }}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <input type="date" value={ev.date}
                               onChange={e => {
                                 const u = [...form.events]; u[i] = { ...u[i], date: e.target.value }; set("events", u);
                               }}
                               className="rounded-lg border px-3 py-2 text-sm outline-none"
                               style={{ background: "#1a3a2a", borderColor: "#234d38", color: "#FAF6EF" }} />
                        <input value={ev.venue}
                               onChange={e => {
                                 const u = [...form.events]; u[i] = { ...u[i], venue: e.target.value }; set("events", u);
                               }}
                               placeholder="Venue"
                               className="rounded-lg border px-3 py-2 text-sm outline-none"
                               style={{ background: "#1a3a2a", borderColor: "#234d38", color: "#FAF6EF" }} />
                      ...
                      <button onClick={() => set("events", form.events.filter((_, j) => j !== i))}
                              className="text-xs flex items-center gap-1" style={{ color: "#b8b4aa" }}>
                        <Trash2 size={11} /> Remove
                      </button>
                    </div>
                  ))}
                  <button onClick={() => set("events", [...form.events, { ...EMPTY_EVENT }])}
                          className="flex items-center gap-2 text-sm font-medium" style={{ color: accent }}>
              ...
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <div className="flex items-center rounded-xl border px-3 text-sm flex-1"
                         style={{ background: "#0f1f17", borderColor: "#234d38" }}>
                      <span className="shrink-0 text-xs" style={{ color: "#b8b4aa" }}>
                        cards.noorapp.app/i/
                      </span>
                      <input
                        value={form.slug}
              ...
              <div className="space-y-5">
                <div className="rounded-xl p-4 space-y-2"
                  ...
                  {[
                    ...
                    { label: "Date",     value: form.weddingDate
                      ? new Date(`${form.weddingDate}T${form.weddingTime}`).toLocaleDateString("en-US", {
        ...
        <div className="flex gap-3 mt-6">
          ...
          {step < STEPS.length - 1 && (
            <button onClick={next} disabled={!canNext()}
                    className="flex-1 flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-semibold disabled:opacity-40"
                    style={{ background: accent, color: "#0f1f17" }}>
  ...
  const previewContent = (
    <>
      {/* Sticky header inside the scrollable panel */}
      <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-3"
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold" style={{ color: "#FAF6EF" }}>Live Preview</span>
        </div>
        <span className="text-xs" style={{ color: "#b8b4aa" }}>Updates live as you type</span>
      </div>

      {/* Desktop: transform scale creates containing block so position:fixed in template
          is relative to this container, keeping MusicPlayer inside the preview frame. */}
      <div className="hidden lg:block"
           style={{ transform: "scale(0.76)", transformOrigin: "top center" }}>
        <PreviewRenderer templateId={templateId} form={form} />
      </div>
      ...
      <p className="hidden lg:block text-center text-xs pt-1 pb-8"
         style={{ color: "#b8b4aa40" }}>
  ...
  /* ── Render ────────────────────────────────────────────────── */
  return (
    <div style={{ background: "#0f1f17" }}>

      {/* ── Mobile tab bar ───────────────────────────── */}
      <div className="lg:hidden sticky top-0 z-50 flex"
           style={{ background: "#0f1f17", borderBottom: "1px solid #234d38" }}>
      ...
      {/* ── Split layout ─────────────────────────────── */}
      {/* On desktop: flex h-screen so both panels scroll independently.
          On mobile: stacked, tab controls visibility. */}
      <div className="lg:flex lg:h-screen">

        {/* Left — form */}
        <div className={`lg:w-1/2 lg:overflow-y-auto ${mobileTab === "preview" ? "hidden lg:block" : "block"}`}>
          {formContent}
        </div>

        {/* Right — live preview */}
        <div className={`lg:w-1/2 lg:overflow-y-auto ${mobileTab === "edit" ? "hidden lg:block" : "block min-h-screen"}`}
             style={{ background: "#040a07", borderLeft: "1px solid #1a3a2a" }}>
          {previewContent}
        </div>
